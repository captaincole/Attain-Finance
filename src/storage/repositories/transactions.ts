 ...
 * @param transactions - Transactions to upsert
 ...
 * @param transactionIds - Transaction IDs to delete
 * @param supabaseClient - Optional Supabase client (uses default if not provided)
 */
...
export async function batchUpdateTransactionBudgets(
  ...
  const promises = updates.map((update) =>
    updateTransactionBudgets(update.transactionId, update.budgetIds, supabaseClient)
  );
 ...
 */
export async function deleteTransactionsByUserId(
  userId: string,
  supabaseClient: SupabaseClient<Database>
): Promise<void> {
